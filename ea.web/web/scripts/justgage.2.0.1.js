...
JustGage = function (config) {
    ...
    this.config =
    {
        ...
        valueFontSize: (config.valueFontSize) ? config.valueFontSize : 16,
        ...
        extraLines: (config.extraLines) ? config.extraLines : [],
    ...
    var minX = dx + (widgetW / 10) + (widgetW / 6.666666666666667 * this.config.gaugeWidthScale) / 2 - widgetW * 0.03;
    var minY = dy + widgetH / 1.126760563380282;

    // max
    var maxFontSize = ((widgetH / 16) > 10) ? (widgetH / 16) : 10;
    var maxX = dx + widgetW - (widgetW / 10) - (widgetW / 6.666666666666667 * this.config.gaugeWidthScale) / 2 + widgetW * 0.03;
    ...
    this.params = {
        ...
        labelX: labelX,
        labelY: labelY,
        minFontSize: minFontSize * 1.5,
        minX: minX,
        minY: minY,
        maxFontSize: maxFontSize * 1.5,
        maxX: maxX,
        maxY: maxY
    };
    ...
    this.canvas.customAttributes.pki = function (value, min, max, w, h, dx, dy, gws, line) {
        ...
        if (line) {
            ...
            if (value > (max - min) / 2) {
                me.lineLabel = me.canvas.text(Xo + 10, Yo - 5, value);
            } else if (value < (max - min) / 2) {
                me.lineLabel = me.canvas.text(Xo - 10, Yo - 10, value);
            } else {
                me.lineLabel = me.canvas.text(Xo, Yo - 10, value);
            }

            var color = me.config.extraLines[0].color;
            me.lineLabel.attr({
                "font-size": '16px',
                "font-weight": "bold",
                "font-family": "Arial",
    ...
    for (var i = 0; i < this.config.extraLines.length; i++) {
        var line = this.config.extraLines[i];
        this.canvas.path().attr({
            "stroke": line.color || '#ff0000',
            "stroke-width": line.width || 1.5,
            pki: [line.value, this.config.min, this.config.max, this.params.widgetW, this.params.widgetH, this.params.dx, this.params.dy, this.config.gaugeWidthScale, true]
        })
    }

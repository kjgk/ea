...
//        alert(result);
    ...
    asyncRenderer: function (url) {
        return function (value, metaData, record) {
            var v = '';
            if (value) {
                v = PageContext._keyNames[value];
                if (!v) {
                    ExtUtil.request({
                        url: PageContext.contextPath + url,
                        params: {
                            objectId: value
                        },
                        method: 'GET',
    ...
    formatNumber: function (v, format) {
        var _format = (format === undefined ? '0,000.00' : format);
        if (v == null || isNaN(v)) {
            return ''
        } else {
            v = Ext.util.Format.number(v, _format) + '';
            var pos = v.indexOf('.');
            while ((v.lastIndexOf('0') != -1 && v.lastIndexOf('0') > pos) || pos == v.length - 1) {
